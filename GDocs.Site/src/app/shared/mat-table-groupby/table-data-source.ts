...
import { MatGroupBy, Group } from './groupBy';
...
export class MatTableDataSource<T> extends DataSource<(T | Group)> {
   ...
   */
  get groupBy(): MatGroupBy<T> | null { return this._groupBy; }
  set groupBy(groupBy: MatGroupBy<T> | null) {
    this._groupBy = groupBy;
    this._updateChangeSubscription();
  }
  private _groupBy: MatGroupBy<T> | null;
  ...
  _pageData(data: (T | Group)[]): (T | Group)[] {
    if (!this.paginator) {
      return this._groupData(data as T[]);
    }

    const startIndex = this.paginator.pageIndex * this.paginator.pageSize;
    const _dataSpliced = data.slice().splice(startIndex, this.paginator.pageSize);
    // Watch for ordered data or group changes to provide a grouped set of data.
    return this._groupData(_dataSpliced as T[]);
  }
